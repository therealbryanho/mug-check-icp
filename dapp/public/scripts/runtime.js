let Runtime = {
  ...
  updateRun(newvalue, run){
    ...
    for (const [key,seconds] of Object.entries(this.timeDivisions)){
      if (this[String(run)].time > seconds){
        this[String(key)] = Math.floor(value / seconds)
        value %= seconds
      }
    }
